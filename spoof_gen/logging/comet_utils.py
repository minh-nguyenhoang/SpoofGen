                ...
                try:
                    ...
                    if comet_ml.get_global_experiment() is not None:
                        logger.warning("You have already created a comet \
                                        ...
                                        cause clashes")
                    if 'experiment_key' not in self._comet_config.keys():
                        self._experiment = comet_ml.Experiment(**self._comet_config)
                    else:
                        self._comet_config.pop('project_name', False)
                        self._experiment = comet_ml.ExistingExperiment(**self._comet_config,
                                                       **{"log_code": True,
                                                       "log_graph": True,
                                                       "parse_args": True,
